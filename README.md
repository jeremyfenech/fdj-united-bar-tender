...
- Every request is logged to the console as one JSON line with timestamp, method, URL, parsed payload and idempotency key when supplied. If JSON parsing fails, the raw body is logged instead. Bodies over 1 MiB are rejected to limit memory use.
